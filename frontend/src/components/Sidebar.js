...
import {setFilter, resetFilter, setMoviesToDisplay} from '../features/general/generalSlice'

function Sidebar() {
  const navigate = useNavigate()
  const dispatch = useDispatch()
  const {filter} = useSelector((state) => state.general)
  const {movies, isSuccess} = useSelector((state) => state.movies)
  ...
  useEffect(() => {
    if(filter === 'all') {
      dispatch(setMoviesToDisplay(movies))
    }
    if(filter === 'movies') {
      dispatch(setMoviesToDisplay(movies.filter((movie) => movie.category === 'Movie')))
    } 
    if(filter === 'tv') {
      dispatch(setMoviesToDisplay(movies.filter((movie) => movie.category === 'TV Series')))
    }
    if(filter === 'bookmarks') {
  ...
  return (
    <header className="header">
      <svg className="header__logo"width="33" height="27" xmlns="http://www.w3.org/2000/svg"><path d="m26.463.408 3.2 6.4h-4.8l-3.2-6.4h-3.2l3.2 6.4h-4.8l-3.2-6.4h-3.2l3.2 6.4h-4.8l-3.2-6.4h-1.6a3.186 3.186 0 0 0-3.184 3.2l-.016 19.2a3.2 3.2 0 0 0 3.2 3.2h25.6a3.2 3.2 0 0 0 3.2-3.2V.408h-6.4Z" fill="#FC4747"/></svg>
      <nav className="nav">
        <ul className="nav__list">
          ...
          <li className="nav__link">
            <button
            title="Movies"
            className={filter === 'movies' ? 'selected' : ''} 
            onClick={() => {handleSetFilter('movies')}}>
          ...
          <li className="nav__link">
            <button
            title="Bookmarks"
            className={filter === 'bookmarks' ? 'selected' : ''} 
            onClick={() => {handleSetFilter('bookmarks')}}>
